defmodule KQWeb.IndexLiveTest do
  ...
  describe "as normal user" do
    test "renders a card for each quiz", %{conn: conn} do
      # conn = put_session(conn, "authenticated", "banana")
    ...
    test "does not show 'Hi admin' message when not authenticated", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/")
      refute has_element?(view, "#admin-message", "Hi admin")
    end
  end
  ...
  describe "as admin" do
    ...
    test "shows 'Hi admin' message when authenticated", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/")
      assert has_element?(view, "#admin-message", "Hi admin")
    end
